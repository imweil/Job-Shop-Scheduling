...
为验证本文所提出调度方法的有效性与鲁棒性，本文在弹药调度环境下进行了大量实验评估，并与多种具代表性的基线算法进行了系统对比。实验主要从调度效率、资源利用率、任务完成时间和算法稳定性等维度进行评估，进一步分析各类方法在静态与动态场景下的表现差异。本方法的代码详见：[https://github.com/](https://github.com/)

### 3.5.1 对比算法

为全面评估所提出方法的性能，本节选择了多种具有代表性的对比算法，包括：

1. **贪婪算法（Greedy Algorithm, GA）**  
   本方法采用基于贪婪策略的调度机制。每个待调度弹药单元依次选择当前执行时间最短的工作机床，以最小化局部决策时间，从而提升整体保障效率。

2. **启发式算法（Heuristic-Based Scheduling, HBS）**  
   启发式算法结合任务与资源的先验知识，采用人工设定的优先级函数或规则（如最短加工时间优先、最少剩余任务优先等）进行调度决策。该类方法在部分特定场景中表现稳定，但其通用性与鲁棒性受限，依赖于启发函数设计质量。

3. **时间窗自适应算法（Time-Window Adaptive Matching, TWAM）**  
   该方法在贪婪算法的基础上引入时间窗机制，通过智能体自动选择合适的匹配窗口长度，在窗口内对资源进行动态匹配。具备较强的调度粒度与计算复杂度权衡能力，适用于动态调度环境。

4. **规则自适应选择算法（Rule-Based Adaptive Selection, RBAS）**  
   该方法基于深度强化学习，通过智能体选择匹配策略，从而实现调度优化。

5. **遗传算法（Hybrid Genetic Algorithm, HGA）**  
   本方法基于经典遗传算法，采用混合进化机制，引入邻域搜索与交叉变异算子优化策略。通过深入探索解空间与局部精细优化，在大规模任务场景下表现出更快的收敛速度与更优的调度质量，优于传统遗传算法。

---

### 3.5.2 实验环境

为确保实验结果的可信度与代表性，本文构建了一个舰载机弹药转运作业仿真环境，用以模拟多任务、多资源受限、调度冲突频发的典型军用场景。仿真系统支持任务动态注入、资源状态变更等复杂情境，以满足调度方法通用性与鲁棒性的评估需求。

系统涉及若干类型的弹药，每种弹药需经过多个保障工序。各类保障节点及其数量如下：
...
#### - 标准作业调度场景（静态任务）：
模拟舰载机处于正常出动状态，弹药种类与数量已知，目标为最短时间内完成弹药装配与转运任务。

#### - 突发特情响应场景（动态任务）：
为评估算法在不确定性条件下的稳定性与响应能力，设计三类典型特情：

- **机床故障场景**：执行过程中，随机指定一台关键设备故障，系统需重新调度剩余任务，考验算法的任务重调度能力。
- **新增任务场景**：调度过程中动态注入新任务，模拟战术调整或新增任务需求，评估算法的在线调度性能与响应延迟。
- **高复杂度混合场景**：综合前两种情况，模拟多个设备故障、任务冲突与动态变化并发的复杂场景，全面考察算法鲁棒性与适应性。

所有算法与实验均基于 Python 3.8 实现，实验平台如下：
...
为全面评估各调度算法性能，选取以下两个关键指标：

- **最大完工时间（Makespan）**：定义为所有弹药任务中最晚完成时间，反映整个调度流程总时长。该指标越小表示整体调度效率越高。
- **算法执行时间（Time）**：指算法生成一次调度方案的运行耗时，用于衡量算法的计算复杂度与响应能力。该指标越小表示算法运行效率越高。

上述两个指标分别从效果与效率两个维度评估调度方法的综合性能。
